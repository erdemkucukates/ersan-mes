...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ── ADIM 2: Cloudinary URL'yi PDF Data alanina metin olarak kaydet ──
    // (Airtable attachment Cloudinary'den indiremez — untrusted hesap)
    // (base64 ise 100K char limitini asar)
    // Bu yuzden URL'yi text olarak sakliyoruz, viewer direkt kullanir
    console.log('[upload-pdf] ADIM 2: PDF Data alanina Cloudinary URL kaydediliyor...');
    const patchRes = await fetch(
      `https://api.airtable.com/v0/${BASE}/${encodeURIComponent('Satış Emirleri')}/${recordId}`,
      {method:'PATCH', headers:{'Authorization':`Bearer ${TOKEN}`,'Content-Type':'application/json'},
       body:JSON.stringify({fields:{'PDF Data': cloudinaryUrl}})}
    );
    const patchData = await patchRes.json();
    if (patchData.error) {
      console.log('[upload-pdf] Airtable HATA:', JSON.stringify(patchData.error));
      throw new Error('Airtable: ' + JSON.stringify(patchData.error));
    }
    const savedUrl = patchData.fields?.['PDF Data'] || '';
    console.log('[upload-pdf] Airtable PDF Data kaydedildi:', savedUrl);

    console.log('[upload-pdf] BASARILI — URL:', cloudinaryUrl);
